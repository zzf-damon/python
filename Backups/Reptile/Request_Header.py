...
def request_py(url, code_mode, print_sign=False, write_file=False):
    """
    :param url: 网址
    :param code_mode: 网页编码
    :param print_sign: 是否打印 False
    :param write_file: 是否写成文件 False
    :return: pyquery格式化的网页
    """
    html = requests.get(url, headers=headers)
    html.encoding = code_mode  # document.charset
    html = html.text
    if print_sign: print(html)
    doc = pq(html)
    if write_file:
        f = open('./1.html', mode='w', encoding=code_mode)
        f.write(html)
    return doc
